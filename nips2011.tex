...
\title{Optimal Planning with an Approximate Model-Based Reinforcement Learning}
...
the assumptions of our model are not satisfied.
\end{abstract}
...
In the RL settting, an agent interacts with the environment, optimizing its behavior to
...
specialized algorithms that exploits the structure\cite{ApproxFactor} \cite{SPUDD}. 
...
their methods predict the features of the next state and reward with function approximation
techniques. Despite the difficulty of correct prediction, their methods are biased 
for a stochastic environment. It is possible to have
...
There are no simulating steps for model-free methods and modeling is unnecessary. 
...
\begin{definition}
    ...
    \item $A_i$ is a set actions which are accessible by subtask $M_i$. An action can be either primitive or composite.
...
The subtask policy $\pi_i: S_i \rightarrow A_i$ maps an active state to one of the actions to execute.
...
We illustrate our idea with the task hierarchy (Fig. \ref{fig:taxi}) of taxi domain introduced by Dietterich\cite{MaxQJ}.
...
Assume subtask $Root$ is suboptimal and always invokes subtask $Get$ even when 
the passenger is already in the taxi, the optimality can be guaranteed if subtask $Get$
...
we will have the optimal policy because not matter which decision is made 
by subtask $Root$, the passenger can always be picked up and delivered to the destination.

The above examples provide us two observations.
First, in order to guarantee optimality, subtasks $Get$ and $Put$ need 
to act optimally in regards of the goal of whole problem, not 
the subgoal of each subtask. It implies that we need to seek hierarchical optimality
rather than recursive optimality.

Second, the optimality cannot be guaranteed without the modification of the hierarchy. In the original hierarchy, 
...
\begin{definition}
    ...
    Furthermore, $TC(H)$ is a total leaf cover if it is a leaf cover and all primitive actions are direct or indirect (through child subtasks) accessible for every 
...
to primitive actions. The total leaf cover can be converted from the leaf cover by
...
The conversion increases exploration space because each subtask needs to explore more actions. It may increase 
the time to learn the optimal policy. However, as we shown in our experiment, a good approximation model can effectively increase
the learning rate, the time to learn the optimal policy might still decrease in overall.
...
\begin{theorem}
    ...
    $Q_r^{\pi}$, $Q_c^{\pi}$,and $Q_e^{\pi}$ follow the above equations, we have $Q^{\pi}$ satisfies:
    \begin{equation*}
    Q^{\pi}(i, s, a) = 
    \left\{\begin{array}{ll}
...
\begin{theorem}
    ...
    if $TC(H)$ is a total leaf cover of hierarchy $H$, we have $Q^*(i, s, a) = Q^*(s, a), \forall s \in S_i, a \in A_p, M_i \in TC(H)$
    \label{thm:opt}
\end{theorem}
\textbf{Proof:} 
    Let $\pi_f: S \rightarrow A_p$ be a policy for MDP $M$, we can construct a hierarchical policy $\pi$, such that
...
Unfortunately, the convergence of HORDQ is an open problem of the field (see Conjecture 1 of \cite{HORDQ}).
...
\begin{theorem}
    ...
    if we use the above equations to update the Q-values for all subtasks in $TC(H)$, 
    we have $lim_{t \rightarrow \infty} Q^t(i, s, a) \rightarrow Q^*(i, s, a)$
...
Figure \ref{fig:bus}a shows the school bus domain. School bus starts at school $S$. Its task 
is to pickup all of three passengers marked as $P$ and return to the school.
...
$A$ and $B$. If the bus passes any of construction sites, it will get damaged with probability 1 and has the probability
...
The world is divided by six areas. There are six subtasks $Move(1), Move(2), \dots,$ and $Move(6)$ which move the
...
policy. Subtask $Root$ adopted our approximated model-based method. 
...
The pseudo-reward is shown in the parentheses. The parameters are $\alpha=0.1$ and $\gamma=1$. All algorithms follow $\epsilon$-greedy exploration policy
with $\epsilon = 0.1$. 
}
\label{fig:res}
\end{figure}
...
The combination of our approximated model-based approach and MAXQ learns 
...
The result is shown in Figure \ref{fig:res}. In this case, Sarsa(0) learning has the fastest
...
In this work, we propose an approach to combine the approximated model-based method with the model-free
...
Our result suggests the possibility to incorporate 
