...
continuous MDP problems. Cora thesis \cite{Vlad} incorporated model-based, model-free and the Bayesian active learning to the MAXQ framework.
...
all of the policies of the subtasks need to be optimal. It is thus more robust and allows us to incorporate approximate
...
Second, the optimality cannot be guaranteed without modification of the hierarchy. In the original hierarchy, 
...
able to solve the problem on its own. A way to achieve it is to let subtask $Get$ have access
to action $Putdown$. 
...
the learning rate, so the time to learn the optimal policy might still decrease in overall.
...
\textbf{Proof:} 
    ...
    \begin{equation}
        ...
        \label{eq:MaxIrr}
    \end{equation}
    Note that $a_k \in TC(H)$ because all subtasks which have direct access to primitive actions belonging to $TC(H)$.
...
Based on the results of previous section, we can safely use any approximate model-based method 
...
It also simplifies the learning of the transition and reward functions, since we only needs to learn
the dynamics and reward models of planning variables.
...
the execution, an exact model shall include all variables during the planning process. 
